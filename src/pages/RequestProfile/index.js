...
export default function RequestProfile() {
  ...
  const idParams = routes.params.id;
  ...
  const socket = useMemo(
    () =>
      socketio('https://tipsterbackend.herokuapp.com', {query: {comment: idParams}}),
    [idParams],
  );
  ...
  useEffect(() => {
    async function loadDataUsers() {
      const response = await api.get(`root/${idParams}`);
  ...
  useEffect(() => {
    async function loadComment() {
      ...
      const response = await api.get(`comments/${idParams}`);
  ...
  function handleNavigateReviewStar() {
    navigation.navigate('ReviewsStar', {idParams});
    setVisible(false);
  }
  ...
  async function handleLikeComments(id) {
    ...
    await api
      .post(`comments/${idParams}/like`, {name, photoUrl, comment, likes, stars})
